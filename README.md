...
![Desktop table view](docs/screenshot-desktop.png)
...
assets/                     logo, favicons, home-screen icons
...
The push triggers the **Deploy to GitHub Pages** action, which publishes the site files
(page, scripts, styles, data, assets) as they are. The site is usually updated within one to
two minutes; GitHub's CDN may hold the previous files for up to ten minutes. The service
worker fetches the data from the network first on every load and always revalidates it with
the server, so a reload shows the new data as soon as the CDN serves it; the cached copy is
...
Replace **`assets/logo.png`** (the header logo, any aspect ratio, about 120 px tall; it is shown 40 px tall on desktop and 32 px on phones). If its width-to-height ratio changes, also update the `width` and `height` attributes of the `<img class="logo">` in `index.html`.
...
Below 1400 px the table hides the Pistes column, below 1300 px also Type, below 1200 px also
Confidence, and below 900 px also Top and Country (the flag moves next to the resort name), so
the remaining columns fit without horizontal scrolling. The **Columns** button shows which columns are hidden at the current width;
ticking one forces it on and the choice is remembered on this device. Phones (under 768 px)
...
`docs/screenshot-desktop.png` (1366 px table view) and `docs/screenshot-mobile.png` (375 px card
view) are regenerated by hand; they are not used by the site.
